...
if [ "$HOSTNAME" == "tomoplata-OptiPlex-790" ]; then
	...
	DATAROOT=/storage/Datasets/Geology-NicolasColtice/pytorch_records_cont
	HOME=/home/tgillooly/
...
python -m visdom.server $VISDOM_OPTIONS > visdom.log 2>&1 &
	...
	#--continue_train --which_epoch 1900 --epoch_count 1901 \
	# --high_iter 25 --low_iter 5 \
python train.py --dataroot $DATAROOT --name geo_self_attn_continents --x_size 512 \
